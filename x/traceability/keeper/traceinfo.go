...
// GetTraceinfoByDic returns traceinfo by dic
func (k Keeper) GetTraceinfoByDic(ctx sdk.Context, dic string) (val types.Traceinfo, found bool) {
	// store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.TraceinfoKey))
	var traceinfos []types.Traceinfo
	traceinfos = k.GetAllTraceinfo(ctx)
	// var b types.Traceinfo
	for _, value := range traceinfos {
		if dic == value.Dic {
			val = value
			return val, true
		}
